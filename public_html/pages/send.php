...
<a href="send" class="btn btn-default"><i class="fa fa-chevron-left"> назад</i></a><hr>
<?$data=mysqli_fetch_array(mysqli_query($mysqli,"SELECT * FROM `akcioner` WHERE phone='".phone($_GET['to'])."' LIMIT 1;"));
if($data['id']>0){if($data['id']!=$account['id']){
...
<input type="text" value="<?=htmlspecialchars($data['name']);?>" readonly class="form-control">
</div>
<div class="form-group">
<label>Количество акций для передачи</label>
<input type="number" name="amount" min=0 max=<?=(int)$account['kol_akc'];?> value="0" class="form-control">
</div>
<div class="form-group">
<label>Тип сделки</label>
<select name="type" class="form-control" onchange="$('.kol').hide();if($(this).val()==1)$('.kol').show();"><option value=1>Продажа<option value=2>Дарение</select>
</div>
...
<div class="form-group">
<button type="submit" onclick="return confirm('Передать акции?');" class="btn btn-success">Передать акции</button>
</div>
</form>
<?}}else{?><div class="alert alert-danger">Вы не можете отправить акции себе</div><?}}else{?><div class="alert alert-danger">Акционер не найден</div><?}?>
<?}else{?><form>
<div class="form-group">
