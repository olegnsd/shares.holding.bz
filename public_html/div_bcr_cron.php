...
?>
    ...
    if($data['id']>0){
        ...
        $res=mysqli_query($mysqli,"SELECT a.* FROM akcioner AS a WHERE a.id>'".(int)$data['next']."' AND (a.card!='none' OR a.card!=NULL) AND (a.kol_akc!=NULL OR a.kol_akc!=0) AND a.id<> ALL (SELECT user FROM div_bcr_log WHERE task_id = '".$data['id']."') LIMIT 0,100;");
