...
//$cours_eur = $data->Valute->EUR->Value;
        ...
        <div class="row">
            <div class="col-xs-12">
                <ul class="nav nav-pills">
                    ...
                    <li<? if ($_GET['page'] == 'stock') {
                        ?> class="active"<?
                    } ?>><a href="stock">Биржа</a></li>
                    <!--li><a href="buy">Покупка акций</a></li-->
                    <li<? if ($_GET['page'] == 'send') {
                        ?> class="active"<?
                    } ?>><a href="send">Передача акций</a></li>
                    <li<? if ($_GET['page'] == 'divs') {
                        ?> class="active"<?
                    } ?>><a href="divs">История дивидендов</a></li>
                    <li<? if ($_GET['page'] == 'history') {
                        ?> class="active"<?
                    } ?>><a href="history">История акций</a></li>
                    <li<? if ($_GET['page'] == 'settings') {
                        ?> class="active"<?
                    } ?>><a href="settings">Реквизиты акционера</a></li>
                    <!--li<? if ($_GET['page'] == '2') {
                        ?> class="active"<?
                    } ?>><a href="#">Messages</a></li-->
                    <? if ($account['is_admin']) {
                        ?>
                    <li<? if ($_GET['page'] == 'admin') {
                        ?> class="active"<?
                    } ?>><a href="admin">Администрирование</a></li><?
                    } ?>
                </ul>
                <br>
            </div>
            <div class="col-md-8 col-xs-12">
                ...
                } elseif ($_GET['page'] == 'divs') {
                    include('pages/divs.php');
                } elseif ($_GET['page'] == 'settings') {
                    include('pages/divr.php');
                ...
                } elseif (($_GET['page'] == 'admin') & ($account['is_admin'])) {
                    ?>
                    <hr>
                    <ul class="nav nav-pills">
                        <li<? if ($_GET['admin'] == '') {
                            ?> class="active"<?
                        } ?>><a href="admin">Акционеры</a></li>
                        <li<? if ($_GET['admin'] == 'stock') {
                            ?> class="active"<?
                        } ?>><a href="admin/stock">Биржевые операции</a></li>
                        <li<? if ($_GET['admin'] == 'pay') {
                            ?> class="active"<?
                        } ?>><a href="admin/pay">Выплата дивидендов Киви</a></li>
                        <li<? if ($_GET['admin'] == 'div_bcr') {
                            ?> class="active"<?
                        } ?>><a href="admin/div_bcr">Выплата дивидендов BCR</a></li>
                        <li<? if ($_GET['admin'] == 'sms') {
                            ?> class="active"<?
                        } ?>><a href="admin/sms">Рассылка СМС</a></li>
                        <li<? if ($_GET['admin'] == 'message') {
                            ?> class="active"<?
                        } ?>><a href="admin/message">Сообщение акционерам</a></li>
                        <li<? if ($_GET['admin'] == 'polls') {
                            ?> class="active"<?
                        } ?>><a href="admin/polls">Голосования</a></li>
                    </ul><br>
            ...
            <div class="col-md-4 col-xs-12">
                ...
                <div class="panel panel-default">
                    <div class="panel-body text-center"><b
                                style="font-size:40px;"><?= number_format($kol_all / $em['SUM(`kol_akc`)'] * 100, 5, '.', ' ') ?>
                            %</b><br>ваша доля
                    </div>
                    <? $perc = $kol_all / $em['SUM(`kol_akc`)'] * 100;
                    if ($perc < 1) $perc = 1; ?>
                    <div class="progress"
                         style="margin-bottom: 0;border-top-left-radius: 0;border-top-right-radius: 0;">
                        <div class="progress-bar progress-bar-info progress-bar-striped" role="progressbar"
                             style="width: <?= number_format($perc, 5, '.', '') ?>%">
                        </div>
                    </div>
                </div>
                ...
                <div class="panel panel-default">
                    ...
                    <ul class="list-group">
                        ...
                        }, '%list%', 10, 'admin?next=', $_GET['next'], '`user`=' . $account['id']);
    ...
    <?
        ...
        ?><br><br>
        <div class="alert alert-info">У вас нет акций. Доступ в личный кабинет ограничен</div><br><br><?
    }
}
else {
    ?>
        ...
        <div class="col-md-6 col-md-offset-3">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <form method="post">
                        <div class="form-group"><input id="phone1" type="text" name="phone" class="form-control"
                                                       placeholder="Номер телефона"></div>

                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group"><input type="number" name="pin" class="form-control"
                                                               placeholder="КОД"></div>
                            </div>
                            <div class="col-md-8" id="btn1"><a class="btn btn-block btn-info submit-button"
   onclick="if($(this).hasClass('disabled'))return false;$('#btn1 a').addClass('disabled');
   $('.ajax1').load('ajaxreg.php?btn=1&phone='+encodeURI($('#phone1').val()),function(){$('#btn1 a').removeClass('disabled');});return false;">Забыли
                                    код? Выслать новый</a></div>
                            <div class="col-md-8" id="btn1" >
                        ...
                        <div class="ajax1"></div>
                        <? if ($_POST[phone] | $_POST[pin]) {
                            ?>
                            <div class="alert alert-danger">Введены неверные данные</div><?
                        } ?>

                        <button type="submit" class="btn btn-success submit-button">Войти</button>
                    </form>
                </div>
            </div>
        </div>
